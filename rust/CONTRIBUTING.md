...
```bash
cargo run --bin gen_stub --features stub-gen
```

> **Note:** Stub generation requires Python 3.10+ (pyo3-stub-gen does not
> support 3.9). CI auto-generates stubs and includes them in all wheels
> (including Python 3.9). Run the command above to regenerate locally during
> development.
