...
namespace log_surgeon {namespace {
    ...
    void LALR1Parser<NFAStateType, DFAStateType>::generate_lr0_kernels() {
        ...
        while (!unused_item_sets.empty()) {
            ...
            for (std::map<uint32_t, std::vector<Production*>>::value_type const& kv :
                 m_non_terminals) {
        ...
        for (std::map<std::set<Item>, std::unique_ptr<ItemSet>>::value_type const& kv :
            ...
            for (Item const& item : item_set_ptr->m_closure) {
                if (!item.has_dot_at_end()) {
                    ...
                    assert(
                        item_set_ptr->m_next.find(item.next_symbol()) != item_set_ptr->m_next.end(
                        ));
                    ...
                    if (!std::holds_alternative<bool>(action)) {
                        if (std::holds_alternative<ItemSet*>(action)
                            && std::get<ItemSet*>(action) == item_set_ptr->m_next[item.
                                next_symbol()]) {
                            continue;
                        }
                        ...
                        conflict_msg += std::to_string(
                            item_set_ptr->m_next[item.next_symbol()]->m_index
                        );
                        conflict_msg += " causes ";
                        if (std::holds_alternative<ItemSet*>(action)) {
                            ...
                            conflict_msg
                                    += this->m_lexer.m_id_symbol[std::get<Production*>(action)->
                                        m_head];
                            conflict_msg += "-> {";
    ...
    template <typename NFAStateType, typename DFAStateType>
    auto LALR1Parser<NFAStateType, DFAStateType>::parse_advance(Token& next_token,
                                                                bool* accept)
        -> bool {
