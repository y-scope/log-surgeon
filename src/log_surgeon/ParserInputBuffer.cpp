...
auto ParserInputBuffer::read_is_safe() -> bool {
    if (m_finished_reading_input) { return false; }
...
auto ParserInputBuffer::read(Reader& reader) -> ErrorCode {
    ...
    if (m_last_read_first_half) { read_offset = m_storage.size() / 2; }
    if (ErrorCode err = m_storage.read(reader, read_offset, m_storage.size() / 2, bytes_read);
        ErrorCode::Success != err)
    {
        if (ErrorCode::EndOfFile == err) { m_finished_reading_input = true; }
        return err;
    }
    ...
    // finished
    if (bytes_read < m_storage.size() / 2) { m_finished_reading_input = true; }
    m_pos_last_read_char += bytes_read;
    if (m_pos_last_read_char > m_storage.size()) { m_pos_last_read_char -= m_storage.size(); }
...
auto ParserInputBuffer::get_next_character(unsigned char& next_char) -> ErrorCode {
    ...
    if (m_storage.pos() == m_storage.size()) { m_storage.set_pos(0); }
    next_char = character;
    return ErrorCode::Success;
}
