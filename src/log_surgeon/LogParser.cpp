...
void LogParser::add_rules(SchemaAST const* schema_ast) {
    ...
    for (unique_ptr<ParserAST> const& parser_ast : schema_ast->m_schema_vars) {
        ...
        if (rule->m_name == "timestamp") {
            ...
            add_rule("newLineTimestamp",
                     make_unique<RegexASTCat<RegexNFAByteState>>(
                             std::move(r2), std::move(newline_timestamp_regex_ast)));
        ...
        if (contains_delimiter) {
            ...
            if (ErrorCode::Success != error_code) {
                throw std::runtime_error(schema_ast->m_file_path + ":" +
                                         to_string(rule->m_line_num + 1) + ": error: '" +
                                         rule->m_name +
            ...
            throw std::runtime_error(schema_ast->m_file_path + ":" +
                                     to_string(rule->m_line_num + 1) + ": error: '" + rule->m_name +
                                     "' has regex pattern which contains delimiter '" +
                                     char(delimiter_name) + "'.\n" + indent + line + "\n" + indent +
                                     spaces + arrows + "\n");
        }
        ...
        rule->m_regex_ptr = make_unique<RegexASTCat<RegexNFAByteState>>(
                std::move(delimiter_group), std::move(rule->m_regex_ptr));
...
/// TODO: if the first text is a variable in the no timestamp case you lose the first character
/// to static text since it has no leading delim
/// TODO: switching between timestamped and non-timestamped logs
auto LogParser::parse(std::unique_ptr<LogParserOutputBuffer>& output_buffer,
                      LogParser::ParsingAction& parsing_action) -> ErrorCode {
