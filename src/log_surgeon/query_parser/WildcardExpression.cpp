...
auto WildcardExpressionView::extend_to_adjacent_greedy_wildcards() const -> WildcardExpressionView {
    ...
    bool const next_char_is_greedy_wildcard{
            m_end_idx < m_expression->length() && m_expression->char_is_greedy_wildcard(m_end_idx)
    };
...
auto WildcardExpressionView::surrounded_by_delims_or_wildcards(lexers::ByteLexer const& lexer) const
        -> bool {
