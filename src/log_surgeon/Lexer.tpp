...
auto Lexer<TypedNfaState, TypedDfaState>::scan_with_wildcard(
    ...
    while (true) {
        ...
        TypedDfaState const* next = state->next(next_char).second;
        if (next_char == '\n') {
            m_line++;
            if (m_has_delimiters && !m_match) {
                next = m_dfa->get_root()->next(next_char).second;
            ...
            if (m_match) {
                ...
                if (wildcard == '?') {
                    for (uint32_t byte = 0; byte < cSizeOfByte; byte++) {
                        auto* next_state = state->next(byte).second;
                    ...
                    while (!unvisited_states.empty()) {
                        ...
                        for (uint32_t byte = 0; byte < cSizeOfByte; byte++) {
                            ...
                            TypedDfaState const* next_state = current_state->next(byte).second;
