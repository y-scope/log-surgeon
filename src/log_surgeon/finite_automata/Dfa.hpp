...
private:
     ...
     * @param tag_with_op_to_reg_id Returns the updated map of tags with operations to registers.
    ...
    auto assign_transition_reg_ops(
            ...
            std::unordered_map<tag_id_t, register_id_t>& tag_with_op_to_reg_id
    ) -> std::vector<RegisterOperation>;

    /**
     * Creates a new DFA state based on a set of NFA configurations and adds it to `m_states`.
     * @param configuration_set The set of configurations represented by this DFA state.
     * @param tag_to_final_reg Mapping from tags to final reg
     * @return A pointer to the new DFA state.
     */
    auto new_state(
            ConfigurationSet const& configuration_set,
            std::unordered_map<tag_id_t, register_id_t> const& tag_to_final_reg
    ) -> TypedDfaState*;
...
auto Dfa<TypedDfaState, TypedNfaState>::generate(Nfa<TypedNfaState> const& nfa) -> void {
    ...
    std::unordered_map<tag_id_t, register_id_t> final_tag_to_reg;
    auto create_or_get_dfa_state = [this, &dfa_states, &unexplored_sets, &final_tag_to_reg](
        ...
        if (it == dfa_states.end()) {
            ...
            state = new_state(config_set, final_tag_to_reg);
...
auto Dfa<TypedDfaState, TypedNfaState>::assign_transition_reg_ops(
    ...
    for (auto config : closure) {
        for (tag_id_t tag_id{0}; tag_id < tag_with_op_to_reg_id.size(); tag_id++) {
            ...
            if (optional_tag_op.has_value()) {
                ...
                tag_with_op_to_reg_id.try_emplace(tag_id, m_register_handler.add_register());
                if (TagOperationType::Set == tag_op.get_type()) {
                    register_operations.emplace_back(
                            tag_with_op_to_reg_id.at(tag_id),
                ...
                config.set_reg_id(tag_id, tag_with_op_to_reg_id.at(tag_id));
            }
        }
...
auto Dfa<TypedDfaState, TypedNfaState>::new_state(
    ...
    for (auto const configuration : configuration_set) {
        ...
        if (nfa_state->is_accepting()) {
            ...
            for (tag_id_t tag_id{0}; tag_id < tag_to_final_reg.size(); tag_id++) {
