...
auto FileReader::read(char* buf, size_t num_bytes_to_read, size_t& num_bytes_read) -> ErrorCode {
    if (nullptr == m_file) { return ErrorCode::NotInit; }
    if (nullptr == buf) { return ErrorCode::BadParam; }
    num_bytes_read = fread(buf, sizeof(*buf), num_bytes_to_read, m_file);
    if (num_bytes_read < num_bytes_to_read) {
        if (0 != ferror(m_file)) { return ErrorCode::Errno; }
        if (0 != feof(m_file)) {
            if (0 == num_bytes_read) { return ErrorCode::EndOfFile; }
        }
    }
...
auto FileReader::try_open(string const& path) -> ErrorCode {
    ...
    if (nullptr == m_file) {
        if (ENOENT == errno) { return ErrorCode::FileNotFound; }
